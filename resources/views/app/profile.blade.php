...
<body contenteditable="false" ginger_software_stylesheet="true" ginger_software_doc="true">
	...
	<div class="container user">
		...
		<div id="user-badge">
			@if(is_null($data->avatar)  || $data->avatar == '')
				<img src="res/images/default.jpg">
			@else
				<img src="{{$data->avatar}}">
			@endif

			<h2 class="form-signin-heading">{{$data->username}}</h2>

			<div class="label label-info">Registered User</div>
			<p class="member-since">Member since: {{$data->created_at->diffForHumans()}}</p>
			<a href="/user/edit" class="btn btn-info"><i class="fa fa-edit"></i>Edit</a>
		</div>

		<h2>{{$data->username}}'s Videos </h2>
        ...
        @endif
			...
			<div class="clear"></div>
			<a class="user-favorites" href="/user/favourite">View All Favorites</a>
		</div>
